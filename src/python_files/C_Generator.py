...
def create_c_struct_funcs( basetypes, structs, struct_name ):
    ''' create a pure c language struct header '''
    struct_def = structs[ struct_name ]

    ret = funcs_template.format(struct_name)
...
def create_default_gen(basetypes,structs,struct_name,project):
    ...
    ret = ret + '#include "{0}_struct_funcs.h"\n'.format(project['PROJECT'])
...
def create_c_headers( inc_dir, basetypes, structs, struct_order, project_struct):
    ...
    fOut2 = open( inc_dir + os.sep + '{0}_struct_funcs.h'.format(project), "w")

    # FUNCTIONS TO ACT ON STRUCTS
    fOut2.write("#ifndef {0}_FUNCS_H\n".format(project))
    fOut2.write("#define {0}_FUNCS_H\n".format(project))
    fOut2.write("/**\n")
    fOut2.write(" * AutoInterface Generated Code\n" )
    fOut2.write(" * \n" )
    fOut2.write(" *    PROJECT: {0}\n".format(project))
    fOut2.write(" *    VERSION: {0}\n".format(project_struct['VERSION']))
    fOut2.write(" */\n\n")

    fOut2.write( '// Stock Includes\n')
    fOut2.write( '#include <stdlib.h>\n')
    fOut2.write( '#include <stdint.h>\n')
    fOut2.write( '#include <stdio.h>\n')
    fOut2.write( '#include <complex.h>\n')
    fOut2.write( '#include <string.h>\n')
    fOut2.write( '#include "cJSON.h"\n' )
    fOut2.write( '#include "io_utils.h"\n')
    fOut2.write( '#include "{0}_struct_defs.h"\n'.format(project))

    # CPP Guards
    fOut2.write('#ifdef __cplusplus\n')
    fOut2.write('extern "C" {\n')
    fOut2.write('#endif\n')

    fOut2.write('\n')
    for struct_name in struct_order:
        fOut2.write( create_c_struct_funcs( basetypes, structs, struct_name ) )
    ...
    # Header Guard
    fOut2.write( "#endif // Header Guard\n".format(project))
    fOut2.close()


    # STRUCT DEFINTIONS
    fOut = open( inc_dir + os.sep + '{0}_struct_defs.h'.format(project), "w" )
    fOut.write("#ifndef {0}_STRUCT_DEFS_H\n".format(project))
    fOut.write("#define {0}_STRUCT_DEFS_H\n".format(project))
    fOut.write("/**\n")
...
def create_c_impls( src_dir, basetypes, structs, project):
    ...
    project_name = project['PROJECT']
    fOut = open(src_dir + os.sep + '{0}_struct_funcs.c'.format(project_name), "w")
    fOut.write("/**\n")
    fOut.write(" * AutoInterface Generated Code\n" )
    fOut.write(" * \n" )
    fOut.write(" *    PROJECT: {0}\n".format(project_name))
    ...
    fOut.write('#include "cJSON.h"\n')
    fOut.write('#include "io_utils.h"\n')
    fOut.write('#include "{0}_struct_funcs.h"\n'.format(project_name))
